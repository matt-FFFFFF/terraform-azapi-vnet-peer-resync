...
# azapi_resource_action.vnet_peering_tohub_data is triggered by the hub network's address space changing.
...
# terraform_data.vnet_peering_tohub_sync_required is set to either true of false, depending on whether the
# peeringSyncLevel property is LocalNotInSync.
resource "terraform_data" "vnet_peering_tohub_sync_required" {
  input = jsondecode(azapi_resource_action.vnet_peering_tohub_data.output).properties.peeringSyncLevel == "LocalNotInSync"
}

# azapi_resource_action.vnet_peering_tohub_sync is triggered by terraform_data.vnet_peering_tohub_sync_required
...
resource "azapi_resource_action" "vnet_peering_tohub_sync" {
  ...
  body        = local.vnet_peering_tohub_body
  lifecycle {
    replace_triggered_by = [terraform_data.vnet_peering_tohub_sync_required]
  }
}
